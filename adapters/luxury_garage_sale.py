"""
Adapter for Luxury Garage Sale's sale section.
...
This is a multi-brand retailer, so brand is extracted per-product — make sure the brand text matches names in config.BRAND_TIERS (e.g. 'Ralph Lauren' vs 'Polo Ralph Lauren').
"""
from bs4 import BeautifulSoup
from adapters.base import safe_get

SALE_URL = "https://www.luxurygaragesale.com/collections/mens"
SOURCE_NAME = "Luxury Garage Sale"
...
def fetch_deals() -> list:
    ...
    for tile in soup.select(".product-tile"):  # ADJUST ME
        try:
            ...
            print(f"[luxury_garage_sale] skipped a tile due to parse error: {e}")
            continue
